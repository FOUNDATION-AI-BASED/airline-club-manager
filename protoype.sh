...
init_database() {
    ...
    # Blocking mode for Docker build or explicit install blocking
    if [[ "${INSTALL_BLOCKING:-}" == "true" || "${DOCKER_BUILD:-}" == "true" ]]; then
        log "Blocking mode enabled for database init (Docker build or INSTALL_BLOCKING)."
        # Initialize log file
        : > "$INSTALL_DIR/datainit.log"
...
EOF
            ...
            warn "Blocking database initialization failed; see $INSTALL_DIR/datainit.log"
            return 1
        fi
    fi
